...
use models::{
    address::AddressHashMap, Address, BlockHashMap, Operation, OperationType, OperationHashMap, OperationHashSet,
...
impl WrappedOperation {
    ...
    fn get_fee_density(&self) -> Ratio<u64> {
        ...
        // add gas fees
        if let OperationType::ExecuteSC { max_gas, gas_price, ..} = self.op.content.op {
