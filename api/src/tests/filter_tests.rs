...
async fn test_get_graph_interval() {
    ...
    expected.push(block_info(
        block_b.clone(),
        apimodel::StatusInfo::Final,
        Vec::new(),
    ...
    let obtained: serde_json::Value = serde_json::from_slice(res.body()).unwrap();
    let obtained: Vec<BlockInfo> = serde_json::from_value(obtained).unwrap();
    let expected = Vec::<BlockInfo>::new();
    assert_eq!(obtained, expected);
    handle.await.unwrap();
}
...
async fn test_get_block_interval() {
    ...
    assert_eq!(res.status(), 200);
    let obtained: serde_json::Value = serde_json::from_slice(res.body()).unwrap();
    let mut expected: Vec<HashSlot> = Vec::new();
    expected.push((get_test_hash(), get_test_block().header.content.slot).into());
    let expected: serde_json::Value =
        serde_json::from_str(&serde_json::to_string(&expected).unwrap()).unwrap();
    assert_eq!(obtained, expected);
    ...
    assert_eq!(res.status(), 200);
    let obtained: serde_json::Value = serde_json::from_slice(res.body()).unwrap();
    let mut expected = Vec::<HashSlot>::new();
    expected.push(
        (
            block_b
                .header
    ...
    assert_eq!(res.status(), 200);
    let obtained: serde_json::Value = serde_json::from_slice(res.body()).unwrap();
    let obtained: Vec<HashSlot> = serde_json::from_value(obtained).unwrap();
    let mut expected = Vec::<HashSlot>::new();
    expected.push(
        (
            block_a
