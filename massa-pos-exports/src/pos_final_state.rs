...
impl PoSFinalState {
    ...
    pub fn get_address_active_rolls(&self, addr: &Address, cycle: u64) -> Option<u64> {
        ...
        let Some(lookback_cycle) = cycle.checked_sub(3) else {
            return self.initial_rolls.get(addr).cloned();
        };
        let Some(lookback_index) = self.get_cycle_index(lookback_cycle) else {
            return None;
        };
