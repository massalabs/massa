    ...
    Self: Sized + Display,
{
    ...
    fn new_verifiable<Ser: Serializer<Self>, ID: Id>(
        ...
        let hash = self.compute_hash(&content_serialized, &public_key);
    ...
    /// Compute hash
    fn compute_hash(
        &self, // use only for Denounce able object
    ...
    >(
        ...
        let hash = content.compute_hash(&content_serialized, &creator_public_key);
