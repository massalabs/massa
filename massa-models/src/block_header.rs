...
impl SecureShareContent for BlockHeader {
    ...
    fn compute_id<ID: Id>(
        ...
        // ID format: H(content_creator_pub_key | slot | H(nonmalleable_contents_serialized))
        let mut hash_data = Vec::new();
        hash_data.extend(content_creator_pub_key.to_bytes()); // fixed-length prefixes are safer when we don't have separators in the hashed data concatenation
