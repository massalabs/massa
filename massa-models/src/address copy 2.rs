...
#[transition::impl_version(versions("0"), structures("UserAddress", "SCAddress"))]
...
impl Address {
    ...
    fn hash_bytes(&self) -> Vec<u8> {
        match self {
            Address::User(addr) => addr.to_bytes(),
            Address::SC(addr) => addr.to_bytes(),
        }
    }
    ...
    pub fn from_public_key(public_key: &PublicKey) -> Self {
        match public_key {
            PublicKey::PublicKeyV0(pk) => {
                Address::User(UserAddressVariant!["0"](<UserAddress!["0"]>::from_public_key(pk)))
            }
            PublicKey::PublicKeyV1(pk) => {
                Address::User(UserAddressVariant!["1"](<UserAddress!["1"]>::from_public_key(pk)))
            }
        }
    }
...
impl UserAddress {
    ...
    fn to_bytes(&self) -> Vec<u8> {
        match self {
            UserAddress::UserAddressV0(addr) => addr.to_bytes(),
            UserAddress::UserAddressV1(addr) => addr.to_bytes(),
        }
    }
}
...
impl UserAddress {
    fn from_public_key(pk: &PublicKey) -> Self {
        UserAddress(Hash::from_bytes(&pk.a.to_bytes()))
    }
}
...
/*
#[transition::impl_version(versions("1"), structures("UserAddress"))]
impl UserAddress {
...
impl SCAddress {
    fn to_bytes(&self) -> Vec<u8> {
        match self {
            SCAddress::SCAddressV0(addr) => addr.to_bytes(),
            SCAddress::SCAddressV1(addr) => addr.to_bytes(),
        }
    }
}
