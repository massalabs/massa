...
pub(crate) const MAX_ADVERTISE_LENGTH: u32 = 10000;
/// Maximum message length in bytes
pub(crate) const MAX_MESSAGE_SIZE: u32 = 1048576000;
/// Max number of hash in the message `AskForBlocks`
pub(crate) const MAX_ASK_BLOCKS_PER_MESSAGE: u32 = 128;
/// Max number of operations per message
pub(crate) const MAX_OPERATIONS_PER_MESSAGE: u32 = 1024;
/// Length of the handshake random signature
pub(crate) const HANDSHAKE_RANDOMNESS_SIZE_BYTES: usize = 32;
...
lazy_static::lazy_static! {
    ...
    pub(crate)  static ref GENESIS_TIMESTAMP: MassaTime = if cfg!(feature = "sandbox") {
    ...
    pub(crate)  static ref END_TIMESTAMP: Option<MassaTime> = if cfg!(feature = "sandbox") {
        None
    } else {
        Some(1685556000000.into())  // Sunday, April 30, 2023 06:00:00 PM UTC
    };
    /// `KeyPair` to sign genesis blocks.
    pub(crate)  static ref GENESIS_KEY: KeyPair = KeyPair::from_str("S1UxdCJv5ckDK8z87E5Jq5fEfSVLi2cTHgtpfZy7iURs3KpPns8")
        .unwrap();
    /// number of cycle misses (strictly) above which stakers are deactivated
    pub(crate)  static ref POS_MISS_RATE_DEACTIVATION_THRESHOLD: Ratio<u64> = Ratio::new(7, 10);
    /// node version
    pub(crate)  static ref VERSION: Version = {
...
pub(crate) fn get_period_from_args() -> u64 {
...
pub(crate) const ROLL_PRICE: Amount = Amount::from_mantissa_scale(100, 0);
/// Block reward is given for each block creation
pub(crate) const BLOCK_REWARD: Amount = Amount::from_mantissa_scale(3, 1);
/// Cost to store one byte in the ledger
pub(crate) const LEDGER_COST_PER_BYTE: Amount = Amount::from_mantissa_scale(25, 5);
/// Cost for a base entry (address + balance (5 bytes constant))
pub(crate) const LEDGER_ENTRY_BASE_SIZE: usize = ADDRESS_SIZE_BYTES + 8;
/// Cost for a base entry datastore 10 bytes constant to avoid paying more for longer keys
pub(crate) const LEDGER_ENTRY_DATASTORE_BASE_SIZE: usize = 10;
/// Time between the periods in the same thread.
pub(crate) const T0: MassaTime = MassaTime::from_millis(16000);
/// Proof of stake seed for the initial draw
pub(crate) const INITIAL_DRAW_SEED: &str = "massa_genesis_seed";
/// Number of threads
pub(crate) const THREAD_COUNT: u8 = 32;
/// Number of endorsement
pub(crate) const ENDORSEMENT_COUNT: u32 = 16;
/// Threshold for fitness.
pub(crate) const DELTA_F0: u64 = 64 * (ENDORSEMENT_COUNT as u64 + 1);
/// Maximum number of operations per block
pub(crate) const MAX_OPERATIONS_PER_BLOCK: u32 = 5000;
/// Maximum block size in bytes
pub(crate) const MAX_BLOCK_SIZE: u32 = 1_000_000;
/// Maximum capacity of the asynchronous messages pool
pub(crate) const MAX_ASYNC_POOL_LENGTH: u64 = 10_000;
/// Maximum data size in async message
pub(crate) const MAX_ASYNC_MESSAGE_DATA: u64 = 1_000_000;
/// Maximum operation validity period count
pub(crate) const OPERATION_VALIDITY_PERIODS: u64 = 10;
/// cycle duration in periods
pub(crate) const PERIODS_PER_CYCLE: u64 = 128;
...
pub(crate) const POS_SAVED_CYCLES: usize = 7;
/// Number of cycle draws saved in the selector cache
///
/// 5 to have a C-2 to C+2 range (6 cycles post-bootstrap give 5 cycle draws)
/// 1 for margin
pub(crate) const SELECTOR_DRAW_CACHE_SIZE: usize = 6;
/// Maximum size batch of data in a part of the ledger
pub(crate) const LEDGER_PART_SIZE_MESSAGE_BYTES: u64 = 1_000_000;
/// Maximum async messages in a batch of the bootstrap of the async pool
pub(crate) const ASYNC_POOL_BOOTSTRAP_PART_SIZE: u64 = 100;
/// Maximum proof-of-stake deferred credits in a bootstrap batch
pub(crate) const DEFERRED_CREDITS_BOOTSTRAP_PART_SIZE: u64 = 100;
/// Maximum executed ops per slot in a bootstrap batch
pub(crate) const EXECUTED_OPS_BOOTSTRAP_PART_SIZE: u64 = 10;
/// Maximum number of consensus blocks in a bootstrap batch
pub(crate) const CONSENSUS_BOOTSTRAP_PART_SIZE: u64 = 50;
/// Maximum number of consensus block ids when sending a bootstrap cursor from the client
pub(crate) const MAX_CONSENSUS_BLOCKS_IDS: u64 = 300;
/// Maximum size of proof-of-stake rolls
pub(crate) const MAX_ROLLS_COUNT_LENGTH: u64 = 10_000;
/// Maximum size of proof-of-stake production stats
pub(crate) const MAX_PRODUCTION_STATS_LENGTH: u64 = 10_000;
/// Maximum size proof-of-stake deferred credits
pub(crate) const MAX_DEFERRED_CREDITS_LENGTH: u64 = 10_000;
/// Maximum size of executed ops
pub(crate) const MAX_EXECUTED_OPS_LENGTH: u64 = 1_000;
/// Maximum size of executed ops changes
pub(crate) const MAX_EXECUTED_OPS_CHANGES_LENGTH: u64 = 20_000;
/// Maximum length of a datastore key
pub const MAX_DATASTORE_KEY_LENGTH: u8 = 255;
/// Maximum length of an operation datastore key
pub(crate) const MAX_OPERATION_DATASTORE_KEY_LENGTH: u8 = MAX_DATASTORE_KEY_LENGTH;
/// Maximum length of a datastore value
pub(crate) const MAX_DATASTORE_VALUE_LENGTH: u64 = 10_000_000;
/// Maximum length of a datastore value
pub(crate) const MAX_BYTECODE_LENGTH: u64 = 10_000_000;
/// Maximum length of an operation datastore value
pub(crate) const MAX_OPERATION_DATASTORE_VALUE_LENGTH: u64 = 500_000;
/// Maximum ledger changes in a block
pub(crate) const MAX_LEDGER_CHANGES_PER_SLOT: u32 = u32::MAX;
/// Maximum production events in a block
pub(crate) const MAX_PRODUCTION_EVENTS_PER_BLOCK: u32 = u32::MAX;
/// Maximum ledger changes count
pub(crate) const MAX_LEDGER_CHANGES_COUNT: u64 =
    100_u32.saturating_mul(MAX_LEDGER_CHANGES_PER_SLOT) as u64;
/// Maximum number of key/values in the datastore of a ledger entry
pub(crate) const MAX_DATASTORE_ENTRY_COUNT: u64 = u64::MAX;
/// Maximum number of key/values in the datastore of a `ExecuteSC` operation
pub(crate) const MAX_OPERATION_DATASTORE_ENTRY_COUNT: u64 = 128;
...
pub(crate) const MAX_RNG_SEED_LENGTH: u32 =
    PERIODS_PER_CYCLE.saturating_mul(THREAD_COUNT as u64) as u32;
// ***********************
// Bootstrap constants
//

/// Max message size for bootstrap
pub(crate) const MAX_BOOTSTRAP_MESSAGE_SIZE: u32 = 1048576000;
/// Max number of blocks we provide/ take into account while bootstrapping
pub(crate) const MAX_BOOTSTRAP_BLOCKS: u32 = 1000000;
/// max bootstrapped cliques
pub(crate) const MAX_BOOTSTRAP_CLIQUES: u32 = 1000;
/// max bootstrapped dependencies
pub(crate) const MAX_BOOTSTRAP_DEPS: u32 = 1000;
/// Max number of child nodes
pub(crate) const MAX_BOOTSTRAP_CHILDREN: u32 = 1000;
/// Max number of cycles in PoS bootstrap
pub(crate) const MAX_BOOTSTRAP_POS_CYCLES: u32 = 5;
/// Max number of address and random entries for PoS bootstrap
pub(crate) const MAX_BOOTSTRAP_POS_ENTRIES: u32 = 1000000000;
/// Max async pool changes
pub(crate) const MAX_BOOTSTRAP_ASYNC_POOL_CHANGES: u64 = 100_000;
/// Max bytes in final states parts
pub(crate) const MAX_BOOTSTRAP_FINAL_STATE_PARTS_SIZE: u64 = 1_000_000_000;
/// Max size of the IP list
pub(crate) const IP_LIST_MAX_SIZE: usize = 10000;
/// Size of the random bytes array used for the bootstrap, safe to import
pub(crate) const BOOTSTRAP_RANDOMNESS_SIZE_BYTES: usize = 32;
/// Max size of the printed error
pub(crate) const MAX_BOOTSTRAP_ERROR_LENGTH: u64 = 10000;

/// Protocol controller channel size
pub(crate) const PROTOCOL_CONTROLLER_CHANNEL_SIZE: usize = 1024;
/// Protocol event channel size
pub(crate) const PROTOCOL_EVENT_CHANNEL_SIZE: usize = 1024;
/// Pool controller channel size
pub(crate) const POOL_CONTROLLER_CHANNEL_SIZE: usize = 1024;
...
pub(crate) const MAX_GAS_PER_BLOCK: u64 = u32::MAX as u64;
/// Maximum of GAS allowed for asynchronous messages execution on one slot
pub(crate) const MAX_ASYNC_GAS: u64 = 1_000_000_000;
...
pub(crate) const MAX_ENDORSEMENTS_PER_MESSAGE: u32 = 1024;
/// node send channel size
pub(crate) const NODE_SEND_CHANNEL_SIZE: usize = 10_000;
/// max duplex buffer size
pub(crate) const MAX_DUPLEX_BUFFER_SIZE: usize = 1024;
/// network controller communication channel size
pub(crate) const NETWORK_CONTROLLER_CHANNEL_SIZE: usize = 10_000;
/// network event channel size
pub(crate) const NETWORK_EVENT_CHANNEL_SIZE: usize = 10_000;
/// network node command channel size
pub(crate) const NETWORK_NODE_COMMAND_CHANNEL_SIZE: usize = 10_000;
/// network node event channel size
pub(crate) const NETWORK_NODE_EVENT_CHANNEL_SIZE: usize = 10_000;

//
// Constants used in protocol
//
/// Maximum of time we keep the operations in the storage of the propagation thread
pub(crate) const MAX_OPERATION_STORAGE_TIME: MassaTime = MassaTime::from_millis(60000);
/// Maximum size of channel used for commands in retrieval thread of operations
pub(crate) const MAX_SIZE_CHANNEL_COMMANDS_RETRIEVAL_OPERATIONS: usize = 10000;
/// Maximum size of channel used for commands in propagation thread of operations
pub(crate) const MAX_SIZE_CHANNEL_COMMANDS_PROPAGATION_OPERATIONS: usize = 10000;
/// Maximum size of channel used for commands in retrieval thread of block
pub(crate) const MAX_SIZE_CHANNEL_COMMANDS_RETRIEVAL_BLOCKS: usize = 10000;
/// Maximum size of channel used for commands in propagation thread of block
pub(crate) const MAX_SIZE_CHANNEL_COMMANDS_PROPAGATION_BLOCKS: usize = 10000;
/// Maximum size of channel used for commands in retrieval thread of endorsements
pub(crate) const MAX_SIZE_CHANNEL_COMMANDS_RETRIEVAL_ENDORSEMENTS: usize = 10000;
/// Maximum size of channel used for commands in propagation thread of endorsements
pub(crate) const MAX_SIZE_CHANNEL_COMMANDS_PROPAGATION_ENDORSEMENTS: usize = 10000;
/// Maximum size of channel used for commands in connectivity thread
pub(crate) const MAX_SIZE_CHANNEL_COMMANDS_CONNECTIVITY: usize = 10000;
/// Maximum size of channel used for commands in peers management thread
pub(crate) const MAX_SIZE_CHANNEL_COMMANDS_PEERS: usize = 10000;
/// Maximum size of channel used for commands in peer testers thread
pub(crate) const MAX_SIZE_CHANNEL_COMMANDS_PEER_TESTERS: usize = 10000;
/// Maximum size of channel used to send network events to the operation handler
pub(crate) const MAX_SIZE_CHANNEL_NETWORK_TO_OPERATION_HANDLER: usize = 10000;
/// Maximum size of channel used to send network events to the block handler
pub(crate) const MAX_SIZE_CHANNEL_NETWORK_TO_BLOCK_HANDLER: usize = 10000;
/// Maximum size of channel used to send network events to the endorsement handler
pub(crate) const MAX_SIZE_CHANNEL_NETWORK_TO_ENDORSEMENT_HANDLER: usize = 10000;
/// Maximum size of channel used to send network events to the peer handler
pub(crate) const MAX_SIZE_CHANNEL_NETWORK_TO_PEER_HANDLER: usize = 10000;
/// Maximum number of peer in a announcement list of peer
pub(crate) const MAX_PEERS_IN_ANNOUNCEMENT_LIST: u64 = 100;
/// Maximum number of listeners for a peer
pub(crate) const MAX_LISTENERS_PER_PEER: u64 = 100;
//
...
pub(crate) const DENUNCIATION_EXPIRE_PERIODS: u64 = PERIODS_PER_CYCLE;
/// Max number of denunciations that can be included in a block header
pub(crate) const MAX_DENUNCIATIONS_PER_BLOCK_HEADER: u32 = 128;
/// Number of roll to remove per denunciation
pub(crate) const ROLL_COUNT_TO_SLASH_ON_DENUNCIATION: u64 = 1;
/// Maximum size of executed denunciations
pub(crate) const MAX_DENUNCIATION_CHANGES_LENGTH: u64 = 1_000;
