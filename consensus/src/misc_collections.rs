...
pub struct DependencyWaitingBlocks {
    ...
    blocked_to_dep: HashMap<Hash, (u64, QueuedBlock, HashSet<Hash>)>, // hash_blocked, (block_blocked, deps)
...
impl DependencyWaitingBlocks {
    ...
    pub fn insert(
    ...
    ) -> Result<HashMap<Hash, QueuedBlock>, ConsensusError> {
        if self.max_size == 0 {
        ...
        // pruning
        let mut removed: HashMap<Hash, QueuedBlock> = HashMap::new();
    ...
    pub fn get(&self, hash: &Hash) -> Option<&QueuedBlock> {
    ...
    pub fn valid_block_obtained(
        &mut self,
        obtained_h: &Hash,
    ) -> Result<(Option<QueuedBlock>, HashSet<Hash>), ConsensusError> {
...
mod tests {
    ...
    fn test_promote() {
        ...
        deps.insert(hash_a, QueuedBlock::FullBLock(block_a), deps_a.clone())
            .unwrap();
        ...
        deps.insert(hash_c, QueuedBlock::FullBLock(block_c), deps_c)
            .unwrap();
    ...
    fn test_insert() {
        ...
        let removed = deps
            .insert(hash_e, QueuedBlock::FullBLock(block_e), deps_e)
            .unwrap();
    ...
    fn test_insert_chain_dependencies() {
        ...
        deps_a.insert(hash_b);
        let removed = deps
            .insert(hash_a, QueuedBlock::FullBLock(block_a), deps_a.clone())
            .unwrap();
        ...
        let removed = deps
            .insert(hash_c, QueuedBlock::FullBLock(block_c), deps_c)
            .unwrap();
        assert_eq!(removed.len(), 0);
