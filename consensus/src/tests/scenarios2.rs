...
use crate::{
    ...
    default_consensus_controller::DefaultConsensusController, random_selector::RandomSelector,
};
...
async fn test_test_parents() {
    ...
    tools::create_and_test_block(
        ...
        0,
        3,
        vec![valid_hasht0s2, hash_genesis_block_th1],
        false,
        false,
    )
    .await;
    ...
    tools::create_and_test_block(
        ...
        1,
        3,
        vec![hash_genesis_block_th0, hash_genesis_block_th0],
        false,
        false,
    )
    .await;
...
async fn test_block_creation() {
    ...
    for (i, &draw) in expected_slots.iter().enumerate() {
        ...
        .await
        {
            Ok(Some(MockProtocolCommand::PropagateBlock { hash, block })) => {
                assert_eq!(draw, 0);
