...
fn verify_received_block(
    ...
    // Block wrapper uses the default signed-hash layout.
    block.verify_signature(None)?;
    ...
    header.verify_signature(sig_chain_id_for_slot(mip_store, config.chain_id, header_ts))?;
