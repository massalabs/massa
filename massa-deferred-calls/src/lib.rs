...
impl DeferredCallRegistry {
    ...
    */
    ...
    /// Returns the base fee stored for a slot, or zero if the slot is uninitialized.
    /// The `min_gas_cost` fallback for uninitialized slots lives in the speculative
    /// layer, where it can be gated on the active execution component version.
