...
impl MipState {
    ...
    /// Advance the state
    /// Can be called multiple times as it will only store what changes the state in history
...
impl MipStore {
    ...
    pub fn update_batch(
        ...
        let guard = self.0.read();
        guard.update_batch(db_batch, db_versioning_batch, between)
    }
...
mod test {
    ...
    fn test_mip_store_db() {
        ...
        // Step 2
        let ms_1_ = advance_state_until(ComponentState::active(), &mi_1);
        ...
        mip_store
            .update_batch(&mut db_batch, &mut db_versioning_batch, between)
            .unwrap();
