...
pub fn get_mip_list() -> [(MipInfo, MipState); 1] {
    let mip_list = [(
        MipInfo {
            ...
            start: MassaTime::from_utc_ymd_hms(2025, 1, 13, 14, 0, 0).unwrap(), // Monday, January 13, 2025 2:00:00 PM UTC
            // Give 1 week for the MIP to be accepted
            timeout: MassaTime::from_utc_ymd_hms(2025, 1, 20, 14, 0, 0).unwrap(), // Monday, January 20, 2025 2:00:00 PM UTC
            activation_delay: MassaTime::from_millis(60 * 60 * 1000), // The MIP will be activated 1 hour after the LockedIn state
        },
        MipState::new(MassaTime::from_utc_ymd_hms(2025, 1, 10, 10, 0, 0).unwrap()), // Friday, January 10, 2025 10:00:00 AM UTC
    )];
