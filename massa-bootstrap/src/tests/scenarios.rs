...
use crate::{BootstrapConfig, BootstrapManager};
...
fn setup_server_for_whitelist_test(
    addr: SocketAddr,
    bootstrap_config: BootstrapConfig,
) -> BootstrapManager {
    let (_, keypair): &(BootstrapConfig, KeyPair) = &BOOTSTRAP_CONFIG_KEYPAIR;

    let mut sender = MockNetworkCommandSender::new();
    let sender2 = MockNetworkCommandSender::new();
    sender.expect_clone().return_once(|| sender2);
...
fn test_bootstrap_whitelist() {
    ...
    let bs_manager = setup_server_for_whitelist_test(addr.clone(), config.clone());
...
fn test_bootstrap_server() {
    ...
    let (_mock_bs_listener, mock_remote_connector) = conn_establishment_mocks();
