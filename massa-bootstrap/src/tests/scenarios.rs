...
lazy_static::lazy_static! {
    pub static ref BOOTSTRAP_CONFIG_KEYPAIR_WL: (BootstrapConfig, KeyPair) = {
        let keypair = KeyPair::generate();
        (get_bootstrap_config_for_wl(keypair.get_public_key()), keypair)
    };
}
...
#[tokio::test]
async fn test_bootstrap_server_for_wl() {
    let (bootstrap_config, keypair): &(BootstrapConfig, KeyPair) = &BOOTSTRAP_CONFIG_KEYPAIR_WL;
    let rolls_path = PathBuf::from_str("../massa-node/base_config/initial_rolls.json").unwrap();
    let genesis_address = Address::from_public_key(&KeyPair::generate().get_public_key());
    let (mut server_selector_manager, server_selector_controller) =
        start_selector_worker(SelectorConfig {
...
// pub fn load_file() -> Vec<SocketAddr> {
//     use std::fs::File;
//     use std::io::prelude::*;
