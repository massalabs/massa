...
/// - node_event_rx died
async fn protocol_controller_fn<NetworkControllerT: 'static + NetworkController>(
    cfg: ProtocolConfig,
    self_node_id: NodeId,
    private_key: PrivateKey,
    mut network_controller: NetworkControllerT,
    ...
    let mut handshake_futures: FuturesUnordered<
        JoinHandle<HandshakeReturnType<NetworkControllerT>>,
    ...
    loop {
        tokio::select! {
            ...
            evt = network_controller.wait_event() => manage_network_event::<NetworkControllerT>(
                evt,
...
async fn manage_network_event<NetworkControllerT: NetworkController + 'static>(
    evt: NetworkEvent<NetworkControllerT::ReaderT, NetworkControllerT::WriterT>,
    running_handshakes: &mut HashSet<ConnectionId>,
    cfg: &ProtocolConfig,
    self_node_id: NodeId,
    private_key: PrivateKey,
    handshake_futures: &mut FuturesUnordered<JoinHandle<HandshakeReturnType<NetworkControllerT>>>,
    ...
    match evt {
        NetworkEvent::NewConnection((connection_id, reader, writer)) => {
            ...
            let handshake_fn_handle = tokio::spawn(async move {
                handshake_fn::<NetworkControllerT>(
                    connection_id,
                    reader,
                    writer,
                    self_node_id,
...
async fn manage_handshake_futures<NetworkControllerT: NetworkController + 'static>(
...
async fn manage_node_event<NetworkControllerT: NetworkController>(
