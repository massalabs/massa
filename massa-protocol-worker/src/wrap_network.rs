...
//TODO: Move in test folder when old is removed
#[cfg(test)]
/// Everything below may be generated by automock
pub struct MockActiveConnectionsTraitWrapper {
    inner: Arc<RwLock<MockActiveConnectionsTrait>>,
}

#[cfg(test)]
impl Clone for MockActiveConnectionsTraitWrapper {
    fn clone(&self) -> Self {
...
impl MockActiveConnectionsTraitWrapper {
    pub fn new() -> MockActiveConnectionsTraitWrapper {
        MockActiveConnectionsTraitWrapper {
            inner: Arc::new(RwLock::new(MockActiveConnectionsTrait::new())),
        }
    }

    /// Guarantees the borrowed &mut is owned exclusively by the closure
    pub fn set_expectations<F: FnOnce(&mut MockActiveConnectionsTrait)>(&mut self, g: F) {
        g(&mut Arc::get_mut(&mut self.inner).unwrap().write())
    }
}
...
impl ActiveConnectionsTrait for MockActiveConnectionsTraitWrapper {
    fn get_nb_in_connections(&self) -> usize {
        self.inner.read().get_nb_in_connections()
    }

    fn get_nb_out_connections(&self) -> usize {
        self.inner.read().get_nb_out_connections()
    }

    fn get_peers_connected(
        &self,
    ) -> HashMap<PeerId, (SocketAddr, PeerConnectionType, Option<String>)> {
        self.inner.read().get_peers_connected()
    }

    fn get_peer_ids_connected(&self) -> HashSet<PeerId> {
        self.inner.read().get_peer_ids_connected()
    }
    ...
    fn shutdown_connection(&mut self, peer_id: &PeerId) {
        self.inner.write().shutdown_connection(peer_id)
    }

    fn get_peers_connections_bandwidth(&self) -> HashMap<String, (u64, u64)> {
        self.inner.read().get_peers_connections_bandwidth()
    }

    fn get_peer_ids_out_connection_queue(&self) -> HashSet<SocketAddr> {
        self.inner.read().get_peer_ids_out_connection_queue()
    }
}
