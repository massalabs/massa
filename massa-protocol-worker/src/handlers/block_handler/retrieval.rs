...
impl RetrievalThread {
    ...
    fn on_block_header_received(&mut self, from_peer_id: PeerId, header: SecuredHeader) {
        ...
        // durably record that this peer sent us data for that block, so that a later
        // attack detection can be attributed to it even if it disconnects in the meantime
        self.cache
            .write()
            .insert_block_sender(&block_id, &from_peer_id);
    ...
    fn mark_block_as_invalid(&mut self, block_id: &BlockId) {
        ...
        // ban the peers that actually sent us data for this block.
        ...
        let peers_to_ban = self.cache.read().get_block_senders(block_id);
        if peers_to_ban.is_empty() {
            warn!(
                "block {} marked as invalid but no peer could be attributed as its sender: not banning anyone",
                block_id
            );
    ...
    fn on_block_operation_list_received(
        ...
        // durably record that this peer sent us data for that block (see `insert_block_sender`)
        self.cache
            .write()
            .insert_block_sender(&block_id, &from_peer_id);
