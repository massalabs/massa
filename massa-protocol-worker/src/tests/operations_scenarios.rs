...
fn test_protocol_does_not_send_invalid_operations_it_receives_to_pool() {
    ...
    pool_controller
        .expect_clone_box()
        .returning(move ||{
            Box::new(MockPoolController::new())
    });
    let mut selector_controller = Box::new(MockSelectorController::new());
    selector_controller
        .expect_clone_box()
...
fn test_protocol_propagates_operations_only_to_nodes_that_dont_know_about_it_indirect_knowledge_via_header(
    ...
    consensus_controller
        .expect_clone_box()
        .returning(move || {
            Box::new(MockConsensusController::new())
    });
    consensus_controller.expect_register_block_header().returning(move |block_id, block| {
        assert_eq!(block_id, block.id);
    });
    consensus_controller.expect_register_block().returning(move |block_id, slot, block_storage, created| {
        assert_eq!(block_id, block.id);
