...
use massa_models::api::{
    AddressInfo, BlockGraphStatus, BlockInfo, BlockInfoContent, EndorsementInfo, OperationInfo,
    SlotAmount,
};
...
impl MassaApiServer for API<ApiV2> {
    ...
    fn get_endorsements(&self, eds: Vec<EndorsementId>) -> RpcResult<Vec<EndorsementInfo>> {
        ...
        if eds.len() as u64 > api_cfg.max_arguments {
            return Err(crate::error::ApiError::BadRequest("too many arguments".into()).into());
        }
