...
use massa_hash::Hash;
use massa_ledger::{destroy_ledger_db, LedgerConfig, LedgerError};
use massa_models::{
    api::EventFilter,
...
fn test_nested_call_gas_usage() {
    ...
    // destroy the ledger db to prevent the test from holding the lock
    destroy_ledger_db();
}
...
fn generate_events() {
    ...
    let (sample_state, _keep) = get_sample_state().unwrap();
    {
        ...
        let (block_id, block) = create_block(
            vec![create_execute_sc_operation(
                sender_private_key,
                sender_public_key,
                event_test_data,
            )
