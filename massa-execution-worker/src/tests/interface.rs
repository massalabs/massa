...
fn test_emit_event_too_large() {
    ...
    let res = interface.generate_event("a".repeat(config.max_event_size_v0).to_string());
    assert!(res.is_ok());
    let res_2 = interface.generate_event("b".repeat(config.max_event_size_v0 + 1).to_string());
    assert!(res_2.is_err());
    println!("res_2: {:?}", res_2);
    if let Err(e) = res_2 {
