...
use massa_models::api::{
    AddressInfo, BlockInfo, CompactAddressInfo, EndorsementInfo, NodeStatus, OperationInfo,
    ReadOnlyExecution,
};
...
pub(crate) async fn run(
    command: Command,
    context: (&Client, &mut Wallet),
    parameters: &[String],
    json: bool,
) -> Result<Box<dyn Output>> {
    let (client, wallet) = context;
    match command {
        ...
        Command::node_get_staking_addresses => match client.private.get_staking_addresses().await {
        ...
        Command::wallet_info => {
            ...
            match client
                ...
                .await
            {
                Ok(addresses_info) => Ok(Box::new(ExtendedWallet::new(wallet, &addresses_info)?)),
