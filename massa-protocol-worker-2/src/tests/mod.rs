...
use crate::{
    handlers::peer_handler::InitialPeers, start_protocol_controller,
};
...
fn basic() {
    ...
    let (_sender_manager1, mut manager1) =
        start_protocol_controller(config1, consensus_controller1, pool_controller1, storage1).expect("Failed to start protocol 1");
    let (_sender_manager2, mut manager2) =
        start_protocol_controller(config2, consensus_controller2, pool_controller2, storage2).expect("Failed to start protocol 2");
