...
use self::messages::{OperationMessageDeserializer, OperationMessageDeserializerArgs, OperationMessageSerializer};
...
impl OperationHandler {
    pub fn new(active_connections: SharedActiveConnections, receiver: Receiver<(PeerId, Vec<u8>)>) -> Self {
        ...
        let operation_retrieval_thread = std::thread::spawn(move || {
            //TODO: Real values
            let operation_message_deserializer = OperationMessageDeserializer::new(OperationMessageDeserializerArgs {
            ...
            loop {
                match receiver.recv() {
                    Ok((peer_id, message)) => {
                        let (rest, message) = operation_message_deserializer.deserialize::<DeserializeError>(&message).unwrap();
                        if !rest.is_empty() {
