...
impl GraphState {
    ...
    pub fn remove_block(
        ...
        }) = self.block_statuses.remove(block_id)
        {
            ...
            return Err(GraphError::ContainerInconsistency(format!("inconsistency inside block statuses removing stale blocks adding {} - block {} is missing", add_block_id, block_id)));
        }
    }

    pub fn list_final_blocks(&self) -> Result<PreHashSet<BlockId>, GraphError> {
    ...
    pub fn mark_final_blocks(
        ...
        for block_id in final_blocks.into_iter() {
            ...
            }) = self.block_statuses.get_mut(&block_id)
            {
                ...
                return Err(GraphError::ContainerInconsistency(format!("inconsistency inside block statuses updating final blocks adding {} - block {} is missing", add_block_id, block_id)));
            }
        }
        Ok(())
    }
