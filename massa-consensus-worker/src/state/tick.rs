...
impl ConsensusState {
    ...
    pub fn slot_tick(&mut self, current_slot: Slot) -> Result<(), ConsensusError> {
        ...
        info!("AURELIEN: Size of save_final_periods {}", self.save_final_periods.len());
        info!("AURELIEN: Size of latest_final_blocks_periods {}", self.latest_final_blocks_periods.len());
        info!("AURELIEN: Size of best_parents {}", self.best_parents.len());
        info!("AURELIEN: Size of block_statuses {}", self.block_statuses.len());
        info!("AURELIEN: Size of incoming_index {}", self.incoming_index.len());
        info!("AURELIEN: Size of waiting_for_slot_index {}", self.waiting_for_slot_index.len());
        info!("AURELIEN: Size of waiting_for_dependencies_index {}", self.waiting_for_dependencies_index.len());
        info!("AURELIEN: Size of discarded_index {}", self.discarded_index.len());
        info!("AURELIEN: Size of to_propagate {}", self.to_propagate.len());
        info!("AURELIEN: Size of attack_attempts {}", self.attack_attempts.len());
        info!("AURELIEN: Size of new_final_blocks {}", self.new_final_blocks.len());
        info!("AURELIEN: Size of new_stale_blocks {}", self.new_stale_blocks.len());
        info!("AURELIEN: Size of final_block_stats {}", self.final_block_stats.len());
        info!("AURELIEN: Size of protocol_blocks {}", self.protocol_blocks.len());
        info!("AURELIEN: Size of stale_block_stats {}", self.stale_block_stats.len());
        info!("AURELIEN: Size of wishlist {}", self.wishlist.len());
        info!("AURELIEN: Size of prev_blockclique {}", self.prev_blockclique.len());
        info!("AURELIEN: Size of storage block length {}", self.storage.get_block_refs().len());
        info!("AURELIEN: Size of storage endos length {}", self.storage.get_endorsement_refs().len());
