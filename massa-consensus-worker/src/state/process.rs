...
impl ConsensusState {
    ...
    fn process_incoming_block(
        ...
        // Validate first; if accepted, transition and try to add to graph.
            ...
            self.blocks_state.get(&block_id)
        {
            ...
            match &res {
                HeaderCheckOutcome::Discard(_reason) => {
                    // `maybe_note_attack_attempt` requires `&mut self`, so it cannot be
                    // called here while `self.blocks_state` is borrowed. It is called in
                    // `transition_incoming_block` after this function returns the outcome.
                }
                _ => {
            ...
            .map(|b_id| {
                if let Some(slot) = self.prev_blockclique.remove(b_id) {
                    ...
                    let (a_block, storage) = match self.blocks_state.get(b_id) {
                        ...
                        _ => {
                            // INVARIANT: blockclique blocks are always Active with Storage.
                            // Reaching this branch means the block graph is in an inconsistent
                            // state, which should be impossible under normal operation.
                            panic!(
                                "blockclique block not found in active blocks \
                                 and/or its operations are missing"
                            )
                        }
                    };
                    new_blocks_metadata.insert(
                        *b_id,
                        ExecutionBlockMetadata {
                            same_thread_parent_creator: a_block.same_thread_parent_creator,
                            storage: Some(storage.clone()),
...
fn create_active_block(infos: &BlockInfos, block_id: BlockId, thread_count: u8) -> BlockStatus {
    BlockStatus::Active {
        a_block: Box::new(ActiveBlock {
            ...
            parents: infos.parents_hash_period.clone(),
        ...
        storage_or_block: StorageOrBlock::Storage(infos.storage.clone()),
    }
}
...
mod tests {
    use super::*;
    use massa_consensus_exports::block_status::{
        BlockStatus, DiscardReason, HeaderOrBlock, StorageOrBlock,
    };
    ...
    #[test]
    fn test_process_action_from_all_statuses() {
        use massa_hash::Hash;
        use massa_models::block_id::BlockId;
        let slot = make_slot();
        let creator = make_address();

        // Helper for test BlockId
        fn test_block_id(seed: &str) -> BlockId {
            BlockId::generate_from_hash(Hash::compute_from(seed.as_bytes()))
        }
        ...
        let status = BlockStatus::WaitingForSlot(HeaderOrBlock::Block {
            id: test_block_id("waiting_for_slot"),
            slot,
            storage: storage.clone(),
        });
        assert!(matches!(
            ProcessAction::from(&status),
            ProcessAction::WaitingForSlot
        ));

        // WaitingForDependencies -> WaitingForDependencies
        let status = BlockStatus::WaitingForDependencies {
            header_or_block: HeaderOrBlock::Block {
                id: test_block_id("waiting_for_deps"),
                slot,
                storage,
            },
            unsatisfied_dependencies: Default::default(),
            sequence_number: 0,
        };
        assert!(matches!(
            ProcessAction::from(&status),
        ...
        // Incoming(Header) is covered by integration tests where a real signed header is built.

        // Incoming(Block) -> IncomingBlock
        let storage = Storage::create_root();
        let block_status = BlockStatus::Incoming(HeaderOrBlock::Block {
            id: test_block_id("incoming_block"),
            slot,
            storage,
        });
        assert!(matches!(
            ProcessAction::from(&block_status),
            ProcessAction::IncomingBlock
        ));

        // Active -> Ignore
        let active_status = BlockStatus::Active {
            a_block: Box::new(ActiveBlock {
                creator_address: creator,
                block_id: test_block_id("active"),
                parents: vec![],
                children: vec![Default::default(); 1],
                descendants: PreHashSet::default(),
                is_final: false,
                slot,
                fitness: 0,
                same_thread_parent_creator: None,
            }),
            storage_or_block: StorageOrBlock::Storage(Storage::create_root()),
        };
        assert!(matches!(
            ProcessAction::from(&active_status),
            ProcessAction::Ignore
        ));
    }
}
