...
use crate::changes::LedgerChanges;
use crate::message::LedgerEntry;
...
/// Represents a final pool of deterministically sorted messages.
/// The final message pool is attached to the output of the latest final slot within the context of massa-final-state.
/// Nodes must bootstrap the final message pool when they join the network.
pub struct FinalAsyncPool {
    /// async pool config
    config: AsyncPoolConfig,
    /// ledger tree, sorted by rentability
